...
- [x] **MCP server for agent/human parity.** A real, documented tool surface (not `window.editor` itself, which is an internal API): ~90 tools giving an AI agent widget-level parity with a human, attaching to your own open app-window over CDP rather than a hidden headless instance. Desktop-only, an "Allow agent access" toggle in Settings. See [`docs/mcp-server.md`](docs/mcp-server.md).
