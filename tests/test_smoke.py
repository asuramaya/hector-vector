...
sys.path.insert(0, str(Path(__file__).resolve().parent))   # tests/ on path → sibling suites
from test_capabilities import check_analyzer_router         # capability/router unit suite
...
def check_render(svg: Path, out_dir: Path) -> None:
    import svg_render
    from PIL import Image
    out = out_dir / "_out.png"
...
def check_clean_trace() -> None:
    ...
    src = Path(_logo_dir.name) / "_logo.png"
    im.save(src)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        out_dir = Path(td)
        svg = check_pixelvec(out_dir)
        check_render(svg, out_dir)
    ...
    check_analyzer_router()   # capability/router suite, imported from test_capabilities
