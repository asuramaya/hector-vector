...
export function createDocks({ editor, measureFit, viewports, renderProcessorPanel, renderLibrary, renderJobsPanel, processorRelevant, cycleBg }) {
  const secKey = (name) => "hv-sec-" + name;
  // A panel folded on the desktop rail must come back folded — but the phone's tabbed sheet
  // force-expands whatever pane it shows (a tab onto a collapsed panel is an empty sheet). Re-read
  // the saved state on the way back to desktop so the phone's override doesn't leak into the rail.
  function syncCollapse() {
    document.querySelectorAll(".rail-section[data-section]").forEach((s) => {
      s.classList.toggle("collapsed", localStorage.getItem(secKey(s.dataset.section)) === "1");
    });
  }
  function wireSectionCollapse(section) {
    ...
    const key = secKey(section.dataset.section);
    ...
    head.addEventListener("click", (e) => {
      ...
      if (section.parentElement?.classList.contains("sheet-tabbed")) return;
  ...
  //      one toggle folds BOTH side docks; empty docks auto-close, fill auto-opens. ----
  {
    ...
    function renderColor() {
      ...
      if (key !== lastColorKey) { editor._recolorTarget = null; editor._globalEditTarget = null; }   // selection changed → leave Recolor/global-edit mode
    ...
    function relayoutDock(side) {
      ...
      // A tabbed sheet is not a stack: there is nothing to split, the separators are display:none, and
      // one pane owns the whole surface. Writing heights here would leave an inline `flex: 0 0 180px`
      // on the pane — which silently WINS over the sheet's stylesheet and clips the panel to 180px.
      if (dock.classList.contains("sheet-tabbed")) { secs.forEach((s) => { s.style.flex = ""; }); return; }
