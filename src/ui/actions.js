...
export const TOOL_WHY = {
  ...
  "tool:mesh":         ["Gradient mesh", "Drag a grid to warp a shape's colour field"],
...
const FIND = {
  ...
  "tool:mesh":       "gradient grid colour color blend patch warp drag",
...
const VERB_FIND = [
  ...
  ["Pathfinder: Outline", "trace edges boundaries unfilled lines segments"],
  ...
  ["Thread text", "link chain overflow continue flow"],
  ["Unthread text", "unlink disconnect separate"],
  ["Add fill layer", "stack another colour color appearance"],
  ["Add fill", "stack second colour color layer appearance"],
  ["Expand fill stack", "bake flatten layers plain paths"],
  ["Reflect", "mirror flip"],
...
const VERB_WHY = [
  ...
  ["Pathfinder: Outline", "Turn every overlap boundary into an unfilled line"],
  ...
  ["Thread text", "Flow overflow from the first box into the second"],
  ["Unthread text", "Stop the overflow flowing onward"],
  ["Add fill layer", "Stack another fill on top, its own colour and opacity"],
  ["Add fill", "Give it a second, independent fill on top"],
  ["Expand fill stack", "Bake the fill layers into plain, separate paths"],
  ["Reflect", "Mirror it"],
  ["Shear", "Slant it"],
